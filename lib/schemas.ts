...
export const qccCaseSaveSchema = z.object({
  ...
  steps: z.array(processStepSchema).min(1).max(20), transitions: z.array(transitionSchema), findings: z.array(z.record(z.unknown())), hypotheses: z.array(z.record(z.unknown())), countermeasures: z.array(z.record(z.unknown())), stage: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5)]), status: z.enum(["草稿", "待诊断", "验证中", "方案设计", "已完成"]), engine: z.enum(["规则引擎", "AI模型", "AI失败后规则引擎"]).optional(), createdAt: z.string(), updatedAt: z.string(), version: z.number().int().positive(),
}).passthrough();
